...
describe('<Input />', () => {
  ...
  it('renders auto generated id when title is passed', () => {
    ...
    expect(screen.getByTestId('sf-form-field-container-title').getAttribute('for')).toBe('input-test');
  });
  ...
  it('uses id over title for input id when both are passed in', () => {
    ...
    expect(screen.getByTestId('sf-form-field-container-title').getAttribute('for')).toBe('test-id');
  });
